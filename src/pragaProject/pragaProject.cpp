        ...
        for (int row = 0; row < meteoGridDbHandler->gridStructure().header().nrRows; row++)
        {
            ...
            for (int col = 0; col < meteoGridDbHandler->gridStructure().header().nrCols; col++)
            {
                if (meteoGridDbHandler->meteoGrid()->getMeteoPointActiveId(row, col, id))
                {
                    ...
                    {
                        result = readClimateElab(db, table.toLower(), climateIndex, QString::fromStdString(meteoPoint->id),
                                                 climateSelected, errorString);
...
bool PragaProject::computeElaboration(bool isMeteoGrid, bool isAnomaly, bool isClimate, bool showInfo)
{
    if (isMeteoGrid)
    {
        ...
        if (! elaborationCycleGridDaily(isAnomaly, showInfo))
        {
            return false;
        }
...
bool PragaProject::elaborationCyclePoints(bool isAnomaly, bool showInfo)
{
    ...
    Crit3DClimate* currentClimate = new Crit3DClimate();

    if (isAnomaly)
    {
        currentClimate->copyParam(referenceClima);
        if (showInfo)
        {
    ...
    {
        currentClimate->copyParam(clima);
        if (showInfo)
        {
    ...
    if (currentClimate->nYears() > 0)
    {
        int myYearStart = currentClimate->yearStart();
        int myYearEnd = currentClimate->yearEnd() + currentClimate->nYears();
        startDate.setDate(myYearStart, currentClimate->genericPeriodDateStart().month(), currentClimate->genericPeriodDateStart().day());
        endDate.setDate(myYearEnd, currentClimate->genericPeriodDateEnd().month(), currentClimate->genericPeriodDateEnd().day());
        // check dates - leap case
        if (currentClimate->genericPeriodDateStart().month() == 2 && currentClimate->genericPeriodDateStart().day() == 29)
        {
            if (!isLeapYear(myYearStart))
            {
                if (currentClimate->periodType() != dailyPeriod)
                {
        ...
        if (currentClimate->genericPeriodDateEnd().month() == 2 && currentClimate->genericPeriodDateEnd().day() == 29)
        {
    ...
    else if (currentClimate->nYears() < 0)
    {
        int myYearStart = currentClimate->yearStart() + currentClimate->nYears();
        int myYearEnd = currentClimate->yearEnd();
        startDate.setDate(myYearStart, currentClimate->genericPeriodDateStart().month(), currentClimate->genericPeriodDateStart().day());
        endDate.setDate(myYearEnd, currentClimate->genericPeriodDateEnd().month(), currentClimate->genericPeriodDateEnd().day());
        // check dates - leap case
        if (currentClimate->genericPeriodDateStart().month() == 2 && currentClimate->genericPeriodDateStart().day() == 29)
    ...
    for (int i = 0; i < meteoPoints.size(); i++)
    {
        if (meteoPoints[i].active)
        {
            ...
            if (isAnomaly && currentClimate->getIsClimateAnomalyFromDb())
            {
                if ( passingClimateToAnomaly(errorString, meteoPointTemp, currentClimate, meteoPoints, clima->getElabSettings()) )
                    nrValidPoints++;
            }
            else
            {
                bool isMeteoGrid = false;
                if ( elaborationOnPoint(errorString, meteoPointsDbHandler, nullptr, meteoPointTemp,
                                        currentClimate, isMeteoGrid, startDate, endDate, isAnomaly, meteoSettings, dataAlreadyLoaded))
                    nrValidPoints++;
            }
            // save result to MP
...
bool PragaProject::elaborationCycleGridDaily(bool isAnomaly, bool showInfo)
{
    int nrValidCells = 0;
    ...
    const bool dataAlreadyLoaded = false;
    ...
    for (int row = 0; row < meteoGridDbHandler->gridStructure().header().nrRows; row++)
    {
        ...
        for (int col = 0; col < meteoGridDbHandler->gridStructure().header().nrCols; col++)
        {
            if (meteoGridDbHandler->meteoGrid()->isMeteoPointActive(row, col))
            {
                ...
                // copy data to temporary meteoPoint
                ...
                if (isAnomaly && currentClimate->getIsClimateAnomalyFromDb())
                {
                    if (passingClimateToAnomalyGrid(myErrorStr, &meteoPointTemp, currentClimate))
                    {
                        nrValidCells++;
                    }
                }
                ...
                {
                    bool isMeteoGrid = true;
                    if  (elaborationOnPoint(myErrorStr, nullptr, meteoGridDbHandler, &meteoPointTemp,
                                           currentClimate, isMeteoGrid, startDate, endDate, isAnomaly,
    ...
    for (int i = 0; i < meteoPoints.size(); i++)
    {
        if (meteoPoints[i].active)
        {
            ...
            for (int j = 0; j < climateList->listClimateElab().size(); j++)
            {
                ...
                bool isMeteoGrid = false;
                if (climateOnPoint(errorString, meteoPointsDbHandler, nullptr, clima, meteoPointTemp,
                                   outputValues, isMeteoGrid, startDate, endDate, changeDataSet, meteoSettings))
                {
                    nrValidCells++;
                }
        ...
        for (int col = 0; col < meteoGridDbHandler->gridStructure().header().nrCols; col++)
        {
           if (meteoGridDbHandler->meteoGrid()->getMeteoPointActiveId(row, col, id))
           {
               ...
               for (int j = 0; j < climateList->listClimateElab().size(); j++)
               {
                   ...
                   bool isMeteoGrid = true;
                   if (climateOnPoint(errorString, nullptr, meteoGridDbHandler, clima, meteoPointTemp,
                                      outputValues, isMeteoGrid, startDate, endDate, changeDataSet, meteoSettings))
                   {
    ...
    if (getUseDetrendingVar (myVar))
    {
        ...
        else
        {
            for (unsigned int i = 0; i < interpolationSettings.getProxyNr(); i++)
            {
                if (interpolationSettings.getSelectedCombination().getActiveList()[i])
                {
                    gis::Crit3DRasterGrid* proxyGrid = interpolationSettings.getProxy(i)->getGrid();
        ...
        for (int col = 0; col < this->meteoGridDbHandler->gridStructure().header().nrCols; col++)
        {
            if (meteoGridDbHandler->meteoGrid()->getMeteoPointActiveId(row, col, id))
            {
                if (!meteoGridDbHandler->gridStructure().isFixedFields())
                {
                    if (myFreq == daily)
                    {
                        meteoGridDbHandler->loadGridDailyDataRowCol(row, col, meteoGridDbHandler->db(), QString::fromStdString(id),
...
void PragaProject::showPointStatisticsWidgetGrid(std::string id)
{
    ...
    bool PragaProject::exportXMLElabGridToNetcdf(QString xmlName)
    {
        ...
        for (int i = 0; i<listXMLElab->listAll().size(); i++)
        {
            ...
            elaborationCycleGridDaily(false, false);
        ...
        for (int i = 0; i<listXMLAnomaly->listAll().size(); i++)
        {
            ...
            elaborationCycleGridDaily(false, false);
            qDebug() << "--------------------------------------------------";
            elaborationCycleGridDaily(true, false);
    ...
    for (int i = 0; i < meteoPoints.size(); i++)
    {
        if (meteoPoints[i].active)
        {
            ...
            for (int i = 0; i<listXMLElab->listAll().size(); i++)
            {
                ...
                if (climateOnPoint(errorString, meteoPointsDbHandler, nullptr, clima, meteoPointTemp,
                                   outputValues, listXMLElab->isMeteoGrid(), startDate, endDate, changeDataSet, meteoSettings))
                {
                    validCell = validCell + 1;
                }
                changeDataSet = false;
...
bool PragaProject::computeRadiationList(const QString &fileName, QString folderString)
{
    ...
    loadIniDateFixed.setNullDate();
    loadEndDateFixed.setNullDate();
    ...
    for (int i = 0; i < radPointsList.size(); i++)
    {
        ...
        while (myDate < myPoint.endDate || (myDate == myPoint.endDate && myHour <= myPoint.endHour))
        {
            ...
            if (radSettings.getLinkeMode() == PARAM_MODE_MONTHLY)
                ...
                myLinke = radSettings.getLinkeDefault();
...
bool PragaProject::shiftMeteoPointsData(bool isAllPoints)
{
    ...
    for (int i = 0; i < meteoPoints.size(); i++)
    {
        ...
        {
            pointList << QString::fromStdString(meteoPoints[i].id);
        }
    }
    ...
    {
        logInfo("No meteo points selected.");
        return false;
    }

    DialogShiftData shiftDialog(getCurrentDate(), isAllPoints);
    ...
    if (shift == 0)
    {
        logInfo("Shift is zero, data are not shifted.");
        return false;
    }

    // read shift parameters
    meteoVariable variable = shiftDialog.getVariable();
    int varId = meteoPointsDbHandler->getIdfromMeteoVar(variable);
    QDate dateFrom = shiftDialog.getDateFrom();
    QDate dateTo = shiftDialog.getDateTo();
    bool isOverwrite = shiftDialog.getOverwrite();

    // set boundary dates
    QDate dateFromShifted = dateFrom.addDays(shift);
    QDate dateToShifted = dateTo.addDays(shift);
    int startIndex = 0;
    if (! isOverwrite)
    {
        if (dateFromShifted > dateTo)
        {
            logError("This operation is not possible without overwriting.");
            return false;
        }
        if (dateToShifted < dateFrom)
        {
            logError("This operation is not possible without overwriting.");
            return false;
        }
        if (dateFromShifted < dateFrom)
        {
            startIndex += dateFromShifted.daysTo(dateFrom);
            dateFromShifted = dateFrom;
            logWarning("Some data will not be shifted: check overwrite.");
        }
        if (dateToShifted > dateTo)
        {
    ...
    reply = QMessageBox::question(nullptr, "Are you sure?" ,
                                  "The data of " + QString::number(pointList.size()) + " points will be " + QString::number(shift) +" days shifted",
    ...
    {
        // load data
        QString idStr = pointList[i];
        QDate firstDateDB;
        std::vector<float> dailyData = meteoPointsDbHandler->loadDailyVar(variable, getCrit3DDate(dateFrom),
                                                                          getCrit3DDate(dateTo), idStr, firstDateDB);
        if (dailyData.empty())
            continue;

        if (firstDateDB != dateFrom)
        {
            logError("Missing first data for id point " + idStr);
            continue;
        }

        // delete old data
        if (! meteoPointsDbHandler->deleteData(idStr, daily, varList, dateFrom, dateTo))
        {
            logError("Failed to delete data for id point " + idStr);
            continue;
        }
        if (isOverwrite)
        {
            if (! meteoPointsDbHandler->deleteData(idStr, daily, varList, dateFromShifted, dateToShifted))
            {
                logError("Failed to delete data for id point " + idStr);
                continue;
            }
        ...
        QDate currentDate = dateFromShifted;
        while (currentDate <= dateToShifted)
        {
            if (index < dailyData.size())
            {
                listEntries.push_back(QString("('%1',%2,%3)").arg(currentDate.toString("yyyy-MM-dd"))
                                      .arg(varId).arg(dailyData[index]));
        ...
        // write data
        if (! meteoPointsDbHandler->writeDailyDataList(idStr, listEntries, errorString))
        {
            logError("Failed to shift data for id point: " + idStr);
            continue;
        }
    }
