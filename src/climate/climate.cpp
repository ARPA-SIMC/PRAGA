                        ...
                        bool isAnomaly, Crit3DMeteoSettings* meteoSettings, bool isDataAlreadyLoaded)
{
    ...
    if (clima->param1IsClimate())
    {
        ...
        if (index != NODATA)
        {
            float param = readClimateElab(clima->db(), table, index, QString::fromStdString(meteoPointTemp->id), clima->param1ClimateField(), errorString);
    ...
    float percValue, result;
    std::vector<float> outputValues;
    bool dataLoaded;

    if (isDataAlreadyLoaded)
    {
        dataLoaded = preElaborationWithoutLoad(meteoPointTemp, meteoSettings, clima->variable(), startDate, endDate,
                                               outputValues, percValue);
    }
    else
    {
...
bool elaborationOnPointHourly(Crit3DMeteoPointsDbHandler* meteoPointsDbHandler, Crit3DMeteoGridDbHandler* meteoGridDbHandler,
                              Crit3DMeteoPoint* meteoPointTemp, bool isMeteoGrid, Crit3DClimate* climate, Crit3DMeteoSettings* meteoSettings, QString &errorString)
{
    ...
    float percValue = loadHourlyVarSeries_SaveOutput(meteoPointsDbHandler, meteoGridDbHandler, QString::fromStdString(meteoPointTemp->id),
                                                     isMeteoGrid, climate->variable(), firstTime, lastTime, outputValues, errorString);
                    ...
                    bool isMeteoGrid, const QDate &startDate, const QDate &endDate, bool changeDataSet, Crit3DMeteoSettings* meteoSettings)
{
    ...
    if (isMeteoGrid)
    {
        climate->setDb(meteoGridDbHandler->db());
    }
    else
    {
        climate->setDb(meteoPointsDbHandler->getDb());
    }

    meteoComputation elab1MeteoComp = getMeteoCompFromString(MapMeteoComputation, climate->elab1().toStdString());
    meteoComputation elab2MeteoComp = getMeteoCompFromString(MapMeteoComputation, climate->elab2().toStdString());

    // check id points
    if (changeDataSet == false)
    {
        // check download data
        if ( (climate->variable() != climate->getCurrentVar() || climate->yearStart() < climate->getCurrentYearStart() || climate->yearEnd() > climate->getCurrentYearEnd()) ||
             (climate->elab1() != climate->getCurrentElab1() && (elab1MeteoComp == correctedDegreeDaysSum || elab1MeteoComp == huglin || elab1MeteoComp == winkler ||  elab1MeteoComp == fregoni) ) )
        {
            changeDataSet = true;
        }
        else
    ...
    if (changeDataSet)
    {
        climate->setCurrentVar(climate->variable());
        climate->setCurrentElab1(climate->elab1());
        climate->setCurrentYearStart(climate->yearStart());
        climate->setCurrentYearEnd(climate->yearEnd());
    ...
    if (dataLoaded)
    {
        if (climateTemporalCycle(errorString, climate, outputValues, meteoPointTemp, elab1MeteoComp, elab2MeteoComp, meteoSettings))
        {
            return true;
        }
    }
                                          ...
                                          std::vector<float> &outputValues, float* percValue)
{
    ...
    for (unsigned int index = 0; index < unsigned(meteoPoint.nrObsDataDaysD); index++)
    {
        ...
        {
        case dailyThomAvg:
            result = thomDailyAvg(meteoPoint.obsDataD[index].tAvg, meteoPoint.obsDataD[index].rhAvg);
            break;
        case dailyThomDaytime:
                result = thomDayTime(meteoPoint.obsDataD[index].tMax, meteoPoint.obsDataD[index].rhMin);
            break;
        case dailyThomNighttime:
                result = thomNightTime(meteoPoint.obsDataD[index].tMin, meteoPoint.obsDataD[index].rhMax);
                break;
        case dailyBIC:
                result = computeDailyBIC(meteoPoint.obsDataD[index].prec, meteoPoint.obsDataD[index].et0_hs);
                break;
        case dailyAirTemperatureRange:
                result = dailyThermalRange(meteoPoint.obsDataD[index].tMin, meteoPoint.obsDataD[index].tMax);
                break;
        case dailyAirTemperatureAvg:
                {
                    ...
                    if (qualityTavg == quality::accepted)
                    {
                        result = meteoPoint.obsDataD[index].tAvg;
                    }
                    else
                    {
                        result = dailyAverageT(meteoPoint.obsDataD[index].tMin, meteoPoint.obsDataD[index].tMax);
                        meteoPoint.obsDataD[index].tAvg = result;
                    }
                    break;
                }
        case dailyReferenceEvapotranspirationHS:
        {
            ...
            {
                result = meteoPoint.obsDataD[index].et0_hs;
            }
            else
            {
                result = dailyEtpHargreaves(meteoPoint.obsDataD[index].tMin, meteoPoint.obsDataD[index].tMax, date, meteoPoint.latitude, meteoSettings);
                meteoPoint.obsDataD[index].et0_hs = result;
            }
            break;
        }
        case dailyHeatingDegreeDays:
        {
            ...
            if (qualityTavg == quality::accepted)
            {
                ...
                {
                    result = DDHEATING_THRESHOLD - meteoPoint.obsDataD[index].tAvg;
                }
            }
            else
            {
            ...
            meteoPoint.obsDataD[index].dd_heating = result;
            break;
        }
        case dailyCoolingDegreeDays:
        {
            ...
            if (qualityTavg == quality::accepted)
            {
                ...
                {
                    result = meteoPoint.obsDataD[index].tAvg - DDCOOLING_SUBTRACTION;
                }
            }
            else
            {
        ...
        if (meteoPoint.getMeteoPointValueDayH(date, hourlyValues))
        {
            switch(myVar)
            {
                case dailyThomHoursAbove:
                    result = thomDailyNHoursAbove(hourlyValues, meteoSettings->getThomThreshold(), meteoSettings->getMinimumPercentage());
                    break;
                case dailyThomMax:
                    result = thomDailyMax(hourlyValues, meteoSettings->getMinimumPercentage());
                    break;
                case dailyThomAvg:
                    result = thomDailyMean(hourlyValues, meteoSettings->getMinimumPercentage());
                    break;
                case dailyLeafWetness:
                    result = dailyLeafWetnessComputation(hourlyValues, meteoSettings->getMinimumPercentage());
                    break;
                case dailyTemperatureHoursAbove:
                    result = temperatureDailyNHoursAbove(hourlyValues, meteoSettings->getTemperatureThreshold(), meteoSettings->getMinimumPercentage());
                    break;
                default:
                    result = NODATA;
                    break;
            ...
            if (! isEqual(result, NODATA)) nrValidValues += 1;
        }
...
bool elaborateDailyAggrVarFromStartDate(meteoVariable myVar, Crit3DMeteoPoint& meteoPoint, Crit3DMeteoSettings* meteoSettings,
                                        const QDate &firstDate, const QDate &lastDate,
                                        std::vector<float> &outputValues, float &percValue)
{
    outputValues.clear();

    QDate firstObsDate = getQDate(meteoPoint.obsDataD[0].date);
    Crit3DQuality qualityCheck;
    int nrValidValues = 0;
    float result;
    ...
    {
        unsigned long index = firstObsDate.daysTo(myDate);
        ...
        case dailyReferenceEvapotranspirationHS:
        {
            ...
            {
                result = dailyEtpHargreaves(meteoPoint.obsDataD[index].tMin, meteoPoint.obsDataD[index].tMax, getCrit3DDate(myDate), meteoPoint.latitude, meteoSettings);
                meteoPoint.obsDataD[index].et0_hs = result;
            }
            break;
        }
...
bool aggregatedHourlyToDaily(meteoVariable myVar, Crit3DMeteoPoint* meteoPoint, const Crit3DDate &dateIni, const Crit3DDate &dateFin, Crit3DMeteoSettings *meteoSettings)
{
    Crit3DDate date;
...
bool preElaborationWithoutLoad(Crit3DMeteoPoint* meteoPoint, Crit3DMeteoSettings* meteoSettings,
                               meteoVariable variable, const QDate& startDate, const QDate& endDate,
                               std::vector<float> &outputValues, float &percValue)
{
    bool isOk = false;
    bool automaticETP = meteoSettings->getAutomaticET0HS();
    ...
    switch(variable)
    {
        case dailyLeafWetness:
        {
            isOk = elaborateDailyAggrVarFromStartDate(dailyLeafWetness, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            break;
        }

        case dailyThomDaytime:
        {
            isOk = elaborateDailyAggrVarFromStartDate(dailyThomDaytime, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            break;
        }

        case dailyThomNighttime:
        {
            isOk = elaborateDailyAggrVarFromStartDate(dailyThomNighttime, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            break;
        }
        case dailyThomAvg: case dailyThomMax: case dailyThomHoursAbove:
        {
            isOk = elaborateDailyAggrVarFromStartDate(variable, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            break;
        }
        case dailyBIC:
        {
            if (automaticETP)
            {
                isOk = elaborateDailyAggrVarFromStartDate(dailyReferenceEvapotranspirationHS, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
                for (int outputIndex = 0; outputIndex<outputValues.size(); outputIndex++)
                {
                    meteoPoint->obsDataD[outputIndex].et0_hs = outputValues[outputIndex];
                }
            ...
            if (isOk)
            {
                isOk = elaborateDailyAggrVarFromStartDate(dailyBIC, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            }
            break;
        }
        ...
        case dailyAirTemperatureRange:
        {
            isOk = elaborateDailyAggrVarFromStartDate(dailyAirTemperatureRange, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            break;
        }

        case dailyAirTemperatureAvg:
        {
            percValue = loadFromMp_SaveOutput(meteoPoint, dailyAirTemperatureAvg, startDate, endDate, outputValues);
            isOk = (percValue > 0);

            if (!isOk && automaticTmed)
            {
                isOk = elaborateDailyAggrVarFromStartDate(dailyAirTemperatureAvg, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            }
            break;
        }
        ...
        case dailyReferenceEvapotranspirationHS:
        {
            percValue = loadFromMp_SaveOutput(meteoPoint, dailyReferenceEvapotranspirationHS, startDate, endDate, outputValues);
            isOk = (percValue > 0);

            if (!isOk && automaticETP)
            {
                isOk = elaborateDailyAggrVarFromStartDate(dailyReferenceEvapotranspirationHS, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            }
            break;
        ...
        case dailyHeatingDegreeDays:
        {
            percValue = loadFromMp_SaveOutput(meteoPoint, dailyHeatingDegreeDays, startDate, endDate, outputValues);
            isOk = (percValue > 0);

            if (!isOk)
            {
                isOk = elaborateDailyAggrVarFromStartDate(dailyHeatingDegreeDays, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            }
            break;
        }
        case dailyCoolingDegreeDays:
        {
            percValue = loadFromMp_SaveOutput(meteoPoint, dailyCoolingDegreeDays, startDate, endDate, outputValues);
            isOk = (percValue > 0);

            if (!isOk)
            {
                isOk = elaborateDailyAggrVarFromStartDate(dailyCoolingDegreeDays, *meteoPoint, meteoSettings, startDate, endDate, outputValues, percValue);
            }
            break;
        }
...
int computeAnnualSeriesOnPointFromDaily(Crit3DMeteoPointsDbHandler* meteoPointsDbHandler, Crit3DMeteoGridDbHandler* meteoGridDbHandler,
                                        Crit3DMeteoPoint* meteoPointTemp, Crit3DMeteoSettings* meteoSettings, const Crit3DClimate& climate,
                                        bool isMeteoGrid, bool isAnomaly, bool isDataAlreadyLoaded, std::vector<float> &outputValues,
                                        std::vector<int> &outputYears, QString& errorString)
{
    int nrValidYears = 0;

    QDate startDate(climate.yearStart(), climate.genericPeriodDateStart().month(), climate.genericPeriodDateStart().day());
    QDate endDate(climate.yearEnd(), climate.genericPeriodDateEnd().month(), climate.genericPeriodDateEnd().day());
    int yearStart = climate.yearStart();
    int yearEnd = climate.yearEnd();

    Crit3DClimate myClimate = climate;
    for (int myYear = yearStart; myYear <= yearEnd; myYear++)
    {
        ...
        myClimate.setYearStart(myYear);
        myClimate.setYearEnd(myYear);

        if (! isDataAlreadyLoaded)
        {
            meteoPointTemp->nrObsDataDaysD = 0; // should be init
        }
        if (myClimate.nYears() < 0)
        {
            startDate.setDate(myYear + myClimate.nYears(), startDate.month(), startDate.day());
        }
        else if (myClimate.nYears() > 0)
        {
            endDate.setDate(myYear + myClimate.nYears(), endDate.month(), endDate.day());
        }
