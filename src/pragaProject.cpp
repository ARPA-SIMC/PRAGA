    ...
    bool PragaProject::exportXMLElabGridToNetcdf(QString xmlName)
    {
        ...
        for (unsigned int i = 0; i<listXMLDrought->listAll().size(); i++)
        {
            ...
            if (listXMLDrought->listIndex()[i] == INDEX_SPI)
            {
                ...
                int nDays = dateStart.daysTo(dateEnd);
                exportMeteoGridToNetCDF(netcdfName, "Standardized Precipitation Index", "SPI", "", getCrit3DDate(dateStart), nDays, "standard_deviation (SPI at "+std::to_string(listXMLDrought->listTimescale()[i])+" month scale)", listXMLDrought->listYearStart()[i], listXMLDrought->listYearEnd()[i]);
            }
            else if (listXMLDrought->listIndex()[i] == INDEX_SPEI )
            {
                ...
                QDate dateEnd(listXMLDrought->listDate()[i].year(),listXMLDrought->listDate()[i].month(),lastDay);
                int nDays = dateStart.daysTo(dateEnd);
                exportMeteoGridToNetCDF(netcdfName, "Standardized Precipitation Evapotranspiration Index", "SPEI", "", getCrit3DDate(dateStart), nDays, "standard_deviation (SPEI at "+std::to_string(listXMLDrought->listTimescale()[i])+" month scale)", listXMLDrought->listYearStart()[i], listXMLDrought->listYearEnd()[i]);
            }
