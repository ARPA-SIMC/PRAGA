...
void Crit3DSynchronicityWidget::addStationGraph()
{
    ...
    dailyValues = meteoPointsDbHandler->loadDailyVar(myVar, getCrit3DDate(myStartDate.addDays(myLag)),
                                                     getCrit3DDate(myEndDate.addDays(myLag)),
                                                     QString::fromStdString(mp.id), firstDaily);
    ...
    dailyRefValues = meteoPointsDbHandler->loadDailyVar(myVar, getCrit3DDate(myStartDate), getCrit3DDate(myEndDate),
                                                        QString::fromStdString(mpRef.id), firstRefDaily);
...
void Crit3DSynchronicityWidget::addInterpolationGraph()
{
    ...
    std::vector<float> dailyValues;
    dailyValues = meteoPointsDbHandler->loadDailyVar(myVar, getCrit3DDate(interpolationStartDate.addDays(myLag)),
                                                     getCrit3DDate(myEndDate.addDays(myLag)),
                                                     QString::fromStdString(mp.id), firstDaily);
    if (dailyValues.empty())
    {
