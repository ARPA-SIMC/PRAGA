...
then
    ...
    # build PRAGA
    cd makeall
    $QMAKE makeall.pro -spec linux-g++-64 CONFIG+=release CONFIG+=force_debug_info CONFIG+=c++17 CONFIG+=qtquickcompiler
    make clean
...
then
    ...
    # build PRAGA
    cd makeall
    $QMAKE makeall.pro -spec linux-g++-64 CONFIG+=release CONFIG+=force_debug_info CONFIG+=c++17 CONFIG+=qtquickcompiler PREFIX=$QT_DIR
    make clean
    make -f Makefile qmake_all 
    make 
